!! User module belonging to test_replicate_L8.par parameter file
!! Had to make a new one because the simulation in lecture 8 uses
!! a density 1.5 in the top half of the domain and 1 in the bottom
!! half.
...
contains
  ...
  subroutine kh_init(ixG^L,ix^L,w,x)
    ...
    double precision :: rhotop, rhobot, uinf, delta0, cn          ! Problem parameters
    logical          :: first = .true.
  ...
  subroutine kh_boundaries(qt,ixI^L,ixO^L,iB,w,x)
    ...
    integer :: ix^D
    double precision :: rhotop, rhobot
